...
public class ManyClause implements Clause, NeedsFutureClause {
    ...
    public void appendSql(DatabaseType databaseType, String str, StringBuilder stringBuilder, List<ArgumentHolder> list) throws SQLException {
        ...
        this.first.appendSql(databaseType, str, stringBuilder, list);
        if (this.second != null) {
            stringBuilder.append(this.operation);
            stringBuilder.append(' ');
            this.second.appendSql(databaseType, str, stringBuilder, list);
        }
        if (this.others != null) {
            for (int i = this.startOthersAt; i < this.others.length; i++) {
                stringBuilder.append(this.operation);
                stringBuilder.append(' ');
                this.others[i].appendSql(databaseType, str, stringBuilder, list);
            }
        }
        stringBuilder.append(") ");
    }

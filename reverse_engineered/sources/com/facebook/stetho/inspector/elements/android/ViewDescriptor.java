...
final class ViewDescriptor extends AbstractChainedDescriptor<View> implements HighlightableDescriptor {
    ...
    class C31641 implements Comparator<ViewCSSProperty> {
        C31641() {
        }
            ...
            synchronized (this) {
                if (this.mViewProperties == null) {
                    ...
                    while (i < length) {
                        ...
                        if (exportedProperty != null) {
                            arrayList.add(new FieldBackedCSSProperty(field, convertViewPropertyNameToCSSName(field.getName()), exportedProperty));
                        }
                        i++;
                    }
                    Collections.sort(arrayList, new C31641());
        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                if ((e instanceof IllegalAccessException) || (e instanceof InvocationTargetException)) {
                    LogUtil.m15278e(e, "failed to get style property " + viewCSSProperty.getCSSName() + " of element= " + view.toString());
                } else {
    ...
    private String convertViewPropertyNameToCSSName(String str) {
        ...
        while (i < split.length) {
            if (!(split[i].equals("get") || split[i].equals(ANSIConstants.ESC_END))) {
            ...
            for (Field field : obj.getClass().getFields()) {
                if (!Modifier.isStatic(field.getModifiers())) {
                    try {
                        ...
                        LogUtil.m15278e(e, "failed to get property of name: \"" + str + "\" of object: " + String.valueOf(obj));
                        return;
                    }
                }
            }

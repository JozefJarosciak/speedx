...
public class Database implements ChromeDevtoolsDomain {
    ...
    private List<Database$DatabaseDriver> mDatabaseDrivers = new ArrayList();
    ...
    public void add(Database$DatabaseDriver database$DatabaseDriver) {
        this.mDatabaseDrivers.add(database$DatabaseDriver);
    }
    ...
    public JsonRpcResult getDatabaseTableNames(JsonRpcPeer jsonRpcPeer, JSONObject jSONObject) throws JsonRpcException {
        ...
        Database$DatabaseDriver databasePeer = getDatabasePeer(getDatabaseTableNamesRequest.databaseId);
        try {
    ...
    public JsonRpcResult executeSQL(JsonRpcPeer jsonRpcPeer, JSONObject jSONObject) {
        ...
        try {
            return getDatabasePeer(str).executeSQL(executeSQLRequest.databaseId, executeSQLRequest.query, new Database$1(this));
        } catch (SQLiteException e) {
    ...
    private Database$DatabaseDriver getDatabasePeer(String str) {
        for (Database$DatabaseDriver database$DatabaseDriver : this.mDatabaseDrivers) {
            List databaseNames = database$DatabaseDriver.getDatabaseNames();

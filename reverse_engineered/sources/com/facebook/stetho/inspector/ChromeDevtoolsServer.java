...
public class ChromeDevtoolsServer implements SimpleEndpoint {
    ...
    static /* synthetic */ class C31471 {
        /* renamed from: $SwitchMap$com$facebook$stetho$inspector$jsonrpc$protocol$JsonRpcError$ErrorCode */
        static final /* synthetic */ int[] f13850x978b462 = new int[ErrorCode.values().length];

        static {
            try {
                f13850x978b462[ErrorCode.METHOD_NOT_FOUND.ordinal()] = 1;
    ...
    public void onOpen(SimpleSession simpleSession) {
        LogRedirector.m15262d(TAG, "onOpen");
    ...
    public void onClose(SimpleSession simpleSession, int i, String str) {
        LogRedirector.m15262d(TAG, "onClose: reason=" + i + " " + str);
    ...
    public void onMessage(SimpleSession simpleSession, byte[] bArr, int i) {
        LogRedirector.m15262d(TAG, "Ignoring binary message of length " + i);
    }

    public void onMessage(SimpleSession simpleSession, String str) {
        if (LogRedirector.isLoggable(TAG, 2)) {
            LogRedirector.m15268v(TAG, "onMessage: message=" + str);
        }
        try {
            ...
            if (LogRedirector.isLoggable(TAG, 2)) {
                LogRedirector.m15268v(TAG, "Unexpected I/O exception processing message: " + e);
            }
            closeSafely(simpleSession, 1011, e.getClass().getSimpleName());
        } catch (MessageHandlingException e2) {
            LogRedirector.m15266i(TAG, "Message could not be processed by implementation: " + e2);
            closeSafely(simpleSession, 1011, e2.getClass().getSimpleName());
        } catch (Throwable e3) {
            LogRedirector.m15269v(TAG, "Unexpected JSON exception processing message", e3);
    ...
    private void handleRemoteMessage(JsonRpcPeer jsonRpcPeer, String str) throws IOException, MessageHandlingException, JSONException {
        ...
        if (jSONObject.has(C0861d.f2145q)) {
            handleRemoteRequest(jsonRpcPeer, jSONObject);
        } else if (jSONObject.has(C0882j.f2229c)) {
    ...
    private static void logDispatchException(JsonRpcException jsonRpcException) {
        ...
        switch (C31471.f13850x978b462[errorMessage.code.ordinal()]) {
            case 1:
                LogRedirector.m15262d(TAG, "Method not implemented: " + errorMessage.message);
                return;
            default:
                LogRedirector.m15271w(TAG, "Error processing remote message", jsonRpcException);
                return;
        }
    }
    ...
    public void onError(SimpleSession simpleSession, Throwable th) {
        LogRedirector.m15264e(TAG, "onError: ex=" + th.toString());
    }
}

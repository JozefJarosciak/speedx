...
public class CrashDumperPlugin implements DumperPlugin {
    ...
    private void doKill(DumperContext dumperContext, Iterator<String> it) throws DumpException {
        ...
        Process start;
        try {
            start = new ProcessBuilder(new String[0]).command(new String[]{"/system/bin/kill", HelpFormatter.DEFAULT_OPT_PREFIX + nextOptionalArg, String.valueOf(Process.myPid())}).redirectErrorStream(true).start();

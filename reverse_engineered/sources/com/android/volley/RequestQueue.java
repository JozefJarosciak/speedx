        ...
        if (request.shouldCache()) {
            synchronized (this.mWaitingRequests) {
                ...
                if (this.mWaitingRequests.containsKey(cacheKey)) {
                    ...
                    if (VolleyLog.DEBUG) {
                        VolleyLog.m3505v("Request for cacheKey=%s is in flight, putting on hold.", cacheKey);
                    }
                } else {
    ...
    <T> void finish(Request<T> request) {
        ...
        if (request.shouldCache()) {
            synchronized (this.mWaitingRequests) {
                ...
                if (queue != null) {
                    if (VolleyLog.DEBUG) {
                        VolleyLog.m3505v("Releasing %d waiting requests for cacheKey=%s.", Integer.valueOf(queue.size()), r2);
                    }

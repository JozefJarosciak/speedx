...
public class DiskBasedCache implements Cache {
    ...
    public DiskBasedCache(File file, long j) {
    ...
    public synchronized void clear() {
        synchronized (this) {
            ...
            this.mEntries.clear();
            this.mTotalSize = 0;
            VolleyLog.m3502d("Cache cleared.", new Object[0]);
        }
    }
        ...
        if (cacheHeader == null) {
            ...
            try {
                ...
                try {
                    ...
                    e = e3;
                    try {
                        VolleyLog.m3502d("%s: %s", fileForKey.getAbsolutePath(), e.toString());
                        remove(str);
                        if (countingInputStream != null) {
                            try {
                                countingInputStream.close();
    ...
    public synchronized void initialize() {
        ...
        if (this.mRootDirectory.exists()) {
            ...
            VolleyLog.m3503e("Unable to create cache dir %s", this.mRootDirectory.getAbsolutePath());
        }
        return;
    ...
    public synchronized void put(String str, Entry entry) {
        ...
        try {
            ...
            if (cacheHeader.writeHeader(bufferedOutputStream)) {
                ...
                bufferedOutputStream.close();
                VolleyLog.m3502d("Failed to write header for %s", fileForKey.getAbsolutePath());
                throw new IOException();
            }
        } catch (IOException e) {
            if (!fileForKey.delete()) {
                VolleyLog.m3502d("Could not clean up file %s", fileForKey.getAbsolutePath());
            }
        }
    }
    ...
    public synchronized void remove(String str) {
        ...
        removeEntry(str);
        if (!delete) {
            VolleyLog.m3502d("Could not delete cache entry for key=%s, filename=%s", str, getFilenameForKey(str));
        }
    }
    ...
    private void pruneIfNeeded(int i) {
        if (this.mTotalSize + ((long) i) >= this.mMaxCacheSizeInBytes) {
            int i2;
            if (VolleyLog.DEBUG) {
                VolleyLog.m3505v("Pruning old cache entries.", new Object[0]);
            }
            ...
            while (it.hasNext()) {
                ...
                if (getFileForKey(cacheHeader.key).delete()) {
                    ...
                    VolleyLog.m3502d("Could not delete cache entry for key=%s, filename=%s", cacheHeader.key, getFilenameForKey(cacheHeader.key));
                }
                it.remove();
                i2 = i3 + 1;
            ...
            i2 = i3;
            if (VolleyLog.DEBUG) {
                VolleyLog.m3505v("pruned %d files, %d bytes, %d ms", Integer.valueOf(i2), Long.valueOf(this.mTotalSize - j), Long.valueOf(SystemClock.elapsedRealtime() - elapsedRealtime));
            }
        }
    }
